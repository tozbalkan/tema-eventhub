    ...
    if (client) {
      ...
      await PgConsumerIdempotencyStore.processIdempotently(
        ...
        async (tx) => {
          ...
          // Read sale lines from PostgreSQL
          const linesRes = await tx.query('SELECT * FROM sale_lines WHERE sale_id = $1', [event.saleId]);
          const assetId = linesRes.rows[0]?.venue_asset_id || 'asset_vip_a1';
          ...
          const assetQuery = `
            ...
            ON CONFLICT (asset_id) DO UPDATE SET
              ...
              last_updated = NOW();
          `;
          await tx.query(assetQuery, [assetId, sale.id, sale.reservation_id, parseFloat(sale.gross_price)]);
