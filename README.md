...
**StageOps (tema-eventhub)** is an enterprise event ticketing and asset reservation platform written in TypeScript and Next.js, featuring a production-grade PostgreSQL persistence layer designed for transaction atomicity, zero overselling, and idempotent business operations under high-concurrency external channel integrations (e.g. Biletix, Passo, Box Office Desk).
...
1. **Database-Authoritative Execution Path**:
   When `cmd.pgClient` is provided, PostgreSQL is the sole authoritative source for asset validation, projection state, pricing data, sale persistence, and outbox messaging.
...
4. **Deterministic Asset Lock Ordering**:
...
- **Multi-Asset Lock Ordering**: Canonical lock ordering prevents circular wait conditions and eliminates PostgreSQL `deadlock detected` errors during concurrent multi-seat sales.
...
The PostgreSQL correctness integration test suite consists of **42 passed integration scenarios** executing against a real PostgreSQL database instance:
